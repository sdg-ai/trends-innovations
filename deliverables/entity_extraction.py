...
#extracting annotations
        ...
        if wiki_data_classes=='true':
            for idx,elem in enumerate(annotations):
                ...
                support = elem["supportLen"]
                tup               = {'entityType': entities,'wiki_classes':classesList,'url':url,'dbPediaIri':dbPediaIri, 'support':support}
                if len(entities):
                    res[word]=tup
        else:
            for idx,elem in enumerate(annotations):
    ...
    def get_annotations(self,doc):
        ...
        for ent in entities_list:
            if ent[0] in wiki_res:
                ...
                ent.append({'entityType':None,'wiki_classes':None,'url':None,'dbPediaIri':None, 'support':0})
